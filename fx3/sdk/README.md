...
The `fx3_debug/`, `fx3_profile_debug/` and `fx3_profile_release/` library variants were
extracted originally but **deleted in Phase 2** — around 45 MB of checkout for libraries
nothing references, since `CMakeLists.txt` only ever links against `fx3_release`. Do not
re-extract them when refreshing this directory.
