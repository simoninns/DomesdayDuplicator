...
The FPGA uses a PLL function (Phase Locked Loop) to generate a single 80MHz system clock from the DE0-Nano's on-board 50MHz clock, and the whole design runs from it. The ADC's 40MHz sampling clock is a divide-by-two of the system clock, so the sampling rate is set by decimating the system clock rather than by a clock of its own. Data from the ADC is collected into a FIFO buffer that is written once per sample and read once per system clock cycle. Because the interface between the FPGA and the Cypress FX3 is clocked at 80MHz — twice the sampling rate — the USB 3.0 interface can ‘catch up’ with the ADC data collection in the event of temporary loss of bandwidth, and it can do so without the two sides sitting in separate clock domains. 

Each 16-bit word the FPGA passes to the USB 3.0 interface carries the 10-bit ADC sample in its low bits and a 6-bit sequence number in its high bits. The sequence number is what lets the capture application detect a sample dropped anywhere between the FPGA and the disk; conversion to scaled 16-bit signed values happens in that application, not in the FPGA. In addition to the data-bus the FPGA provides several additional control signals used to provide flow-control of the data between the FPGA and the Cypress FX3 GPIF interface. 
...
You will now need to program both the FPGA and FX3. Please see the [software guide](software-guide.md) for details.
