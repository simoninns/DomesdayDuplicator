...
`README.md`. A `nix develop` shell supplies all of it already.
...
the repository root, and exactly one `flake.lock`; Nix walks up to find them, so `.#ddd-gui`
resolves identically from `ddd-gui/` and from the root. Components deliberately carry no flake
of their own: an earlier layout gave each one a thin flake for the `cd ddd-gui && nix develop`
