...
GuidedCaptureDialog::GuidedCaptureDialog(AutoCaptureController* controller,
                                         ...
                                         QWidget* parent)
    : QDialog(parent), controller_(controller), disc_(disc) {
  ...
  headline_->setText(ExamineSummary(disc_, player::ExamineOutcome::kCompleted));
...
void GuidedCaptureDialog::BuildAddressControls(QWidget* page,
                                               QFormLayout* form) {
...
int32_t GuidedCaptureDialog::StartAddress() const {
  if (start_frame_ != nullptr) {
...
int32_t GuidedCaptureDialog::EndAddress() const {
...
player::AutoCapturePlan GuidedCaptureDialog::Plan() const {
...
player::PlanProblem GuidedCaptureDialog::problem() const {
  return ValidateAutoCapturePlan(Plan(), disc_);
}

void GuidedCaptureDialog::ApplyShape() {
  // Every control's state is set here from scratch rather than being turned off
  // when a capture starts and left to somebody to turn back on. A window whose
...
QString GuidedCaptureDialog::EstimateText() const {
  const CaptureSettings settings = LoadCaptureSettings();
...
void GuidedCaptureDialog::Refresh() {
  ...
  const player::PlanProblem trouble = problem();

  estimate_->setText(EstimateText());
  problem_->setText(PlanProblemText(trouble));

  start_->setEnabled(!running_ && trouble == player::PlanProblem::kNone &&
                     controller_ != nullptr);
