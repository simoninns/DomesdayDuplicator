...
void CaptureNamingDialog::BuildWidgets() {
  ...
  clear_button_->setObjectName(QLatin1String(kClearButtonName));
  ...
  connect(clear_button_, &QPushButton::clicked, this,
          &CaptureNamingDialog::ClearAllFields);
  ...
  connect(side_spin_, &QSpinBox::valueChanged, this,
          &CaptureNamingDialog::ChangeSide);
}

void CaptureNamingDialog::ShowSettings() {
...
void CaptureNamingDialog::Apply() {
  UpdateEnabledState();
...
void CaptureNamingDialog::ClearAllFields() {
...
void CaptureNamingDialog::ChangeSide(int side) {
  if (!loading_) {
...
void CaptureNamingDialog::UpdateEnabledState() {
...
void CaptureNamingDialog::UpdatePreview() {
  if (controller_ == nullptr) {
    preview_->clear();
    return;
  }
