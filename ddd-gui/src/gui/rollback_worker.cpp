...
RollbackWorker::RollbackWorker(capture::RollbackOrchestrator* orchestrator,
...
RollbackWorker::~RollbackWorker() = default;

void RollbackWorker::Cancel() { cancelled_.store(true); }

void RollbackWorker::Run() {
  if (orchestrator_ == nullptr) {
    emit Finished(false, false, tr("This build cannot program a device."));
    return;
  }
  ...
  const capture::UpdateOutcome outcome =
      task_ == Task::kGateware ? orchestrator_->ProgramGateware(*bundle)
  ...
  // the outcome does not is asked here: was a stop requested? Somebody who
