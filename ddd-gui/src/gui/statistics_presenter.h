...
//
// The gain and the link speed are passed in rather than read from the stats
// block because neither is something the pipeline knows: one is a declaration
// the user made and the other is a property of the device the run was opened
// on.
...
// The view before anything has run — every measured figure blank, and the two
// facts that are known regardless still shown.
