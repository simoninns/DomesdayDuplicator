/************************************************************************
    ...
    Domesday Duplicator - Data conversion

    The 10-bit packed / 16-bit signed sample codec.

    Split out of dataconversion.cpp so it can be unit tested. The conversion itself is
    pure — four samples in, five bytes out, and back — while dataconversion.cpp keeps the
    20 MiB buffering and file I/O around it. A defect here silently corrupts every capture
    that is ever converted, and would not show up as a crash or an error message, so it is
    the highest-value thing in this component to have tests for.
