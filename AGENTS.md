...
**What exists today: 605 tests across five components** — 21 in `gui/` (UTF-8 conversion, the
10-bit/16-bit sample codec), 527 in `ddd-gui/` (the capture engine — sample and wire
...
golden test in `fx3/firmware/` (the generated USB product descriptor). `fpga/` adds five
...
gateware as tested without that caveat. Everything below the top level is simulated.
