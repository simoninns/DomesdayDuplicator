...
2. **The extracted subset is provably sufficient.** Configured and built the firmware against
   ...
   `firmware.img` all produced. So `inc/` + `fx3_release/` + `fx3.ld` + `elf2img` was the
   complete required set at the time. **P5-7 shrank it further**: `elf2img` was replaced by
   project-authored code and removed, so the required vendor subset is now `inc/` +
   three archives from `fx3_release/` + `fx3.ld`, and nothing else.
...
| `util/elf2img/elf2img.c` | Host tool source | ~~Required~~ — **no longer extracted.** P5-7 replaced it with `fx3/mkimage`, project-authored GPLv3 code written against public application note AN76405. Do not restore it |
| `fw_lib/1_3_5/fx3_release/libcy_as0260.a`, `libcy_ov5640.a`, `libcyu3mipicsi.a`, `libcyu3sport.a` | Image-sensor, MIPI-CSI and serial-port libraries | Unused (~1.9 MB). **P5-8 removes them** — removing them left `firmware.img` byte-identical |
...
1. **Extract only the three required rows above.** In particular do not restore `util/` —
   the firmware build no longer references it, and re-adding it would put a proprietary
   artefact back into the build path for no gain. (Until P5-7 this note read
   "`util/elf2img/CMakeLists.txt` is project-authored, not vendor — keep it, or restore it
   afterwards"; that CMake file went with the directory.)
