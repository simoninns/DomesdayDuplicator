...
Where a row names a window, the window is the one the
[capture UX plan](capture-ux-refactor-plan.md) left behind rather than the one that first
discharged the row. That plan built nothing new from this table and retired nothing from it;
it recomposed the surfaces several of these rows landed in — the naming fields became a form
shared by two places, the Player dock's contents moved onto the remote's **Connection** tab,
and the examine-then-set-up chain became one four-page window. The rows are annotated below
where the difference would send a reader to a file that no longer exists.

| Feature (old application) | Notes |
| --- | --- |
| ~~Advanced naming: auto/manual filename modes, disc metadata fields, mint marks, per-side holdings, append-duration rename~~ | **Built** — `capture_naming.h` gained the fields and the rules; the fields themselves are `capture_naming_form.h`, shown by the Capture panel's **Naming…** button and again as the first page of the automatic capture ([capture UX plan](capture-ux-refactor-plan.md), Phases 1 and 4), so the two cannot disagree about naming. The old dialog's three filename modes became a typed name (which wins outright) plus an "include the disc details" option, which is the same three combinations with one fewer control and leaves the Name field meaning what it always meant. Per-side holdings are session-scoped and gated on two options in the form itself rather than in a distant settings tab. The form also grew what the old application had no equivalent of: **Ask the player**, which fills the type, standard and side from the disc without moving it |
...
| ~~Player information display (model, status, position, physical mm)~~ | **Built** — [player control plan](player-control-implementation-plan.md), Task 2.4, as a dock panel. The dock is **gone** ([capture UX plan](capture-ux-refactor-plan.md), Phase 2): the readouts are on the player window's **Connection** tab and the at-a-glance state is the status bar's permanent player label, which is the one surface that cannot be closed. Nothing was dropped — `player_panel.{h,cpp}` retired only once every one of its assertions had moved |
| ~~Player remote dialog (full transport, manual serial commands, user-code reads)~~ | **Built** — [player control plan](player-control-implementation-plan.md), Phase 3. Now tabbed and titled "Player" rather than "Remote control", since it also holds the connection diagnostics: Control, Connection, Disc codes, Manual command ([capture UX plan](capture-ux-refactor-plan.md), Phase 2). It opens whether or not a player is connected, because it is where a user finds out why one is not |
| ~~Automatic capture state machine (whole disc / partial / lead-in, CAV+CLV, key-lock)~~ | **Built** — [player control plan](player-control-implementation-plan.md), Phase 5, re-shaped around a disc examination the old application had no equivalent of. The lead-in is not a shape a player can be asked for; the two shapes that hold it get it by starting the capture before the disc. The window driving it is now `auto_capture_wizard.{h,cpp}` — four pages from examination to summary — and `GuidedCaptureDialog` is retired ([capture UX plan](capture-ux-refactor-plan.md), Phase 4). The state machine itself is untouched: the wizard is one more caller of `AutoCaptureController` |
